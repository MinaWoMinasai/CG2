...
void ObjectDraw::Draw(Renderer renderer, DebugCamera debugCamera, Microsoft::WRL::ComPtr<ID3D12Resource> materialResource, Microsoft::WRL::ComPtr<ID3D12Resource> directionalLightResource, Microsoft::WRL::ComPtr<ID3D12Resource> cameraResource)
{
	suzanneModel_.DrawPro(renderer, worldTransformSuzanne_, debugCamera.GetViewMatrix(), materialResource, directionalLightResource, cameraResource);
	bunnyModel_.DrawPro(renderer, worldTransformBunny_, debugCamera.GetViewMatrix(), materialResource, directionalLightResource, cameraResource);
	teapotModel_.DrawPro(renderer, worldTransformTeapot_, debugCamera.GetViewMatrix(), materialResource, directionalLightResource, cameraResource);
	multiMeshModel_.DrawPro(renderer, worldTransformMultiMesh_, debugCamera.GetViewMatrix(), materialResource, directionalLightResource, cameraResource);
}
